...
try {
  ...
  for (const file of ['src', 'public', 'vite.config.ts', 'popup.html', 'viewer.html', 'offscreen.html']) await cp(resolve('apps/extension', file), resolve(extension, file), { recursive: true });
  ...
  console.log('Environment checks passed: root/workspace loading, public override precedence, private bundle exclusion.');
