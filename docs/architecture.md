...
Rendering helpers stay inside the serialized `installDomControl` function. Each draw reads the current target's computed typography, per-side borders, corner radii and padding, scaled to its visible bounds. Transparent text surfaces composite solid ancestor backgrounds rather than assuming white. Choice previews use a visible associated label or a nearby unambiguous single-control option, falling back to the control itself. Option overlays tint/outline without cloning or covering their text; a separate CSS indicator replaces a visible native control or a unique small decorative marker. Hidden inputs with visible labels remain simulatable. Compact ARIA controls containing words receive an outline rather than an opaque indicator. ARIA radios clear virtual peers only within their nearest explicit radiogroup. These heuristics do not inspect domain names, fetch styles/images, modify original controls or promise exact reproduction of pseudo-elements and complex widgets.
...
`visualPreferences` is a separate local-storage entry validated with defaults of notices off, persistent duration, 3 seconds and purple (`#7871e8`). Temporary duration accepts 0.1–30 seconds in tenths. The `accentColor` field accepts an opaque six-digit hex color; legacy preferences receive the default without losing other fields. Preferences can be saved before hosting and during a host session. A CSS variable on the presentation layer updates existing previews without resetting virtual state. Preferences remain host-local and do not change peer protocol 4. A host-side inactivity timer clears every authorized frame together, including virtual focus. No simulated text is stored. Escape releases gestures without clearing previews; navigation, switching tabs/modes, pause, control withdrawal and shutdown dispose them. Geometry-only rebinding preserves previews in the same document. Drawing follows current rectangles, clips against scroll containers and drops disconnected elements.
