...
**119 unit tests in 13 files passed**, as did workspace TypeScript checks, production compilation and Chrome/Edge packaging. Version 0.4.0 and peer protocol 4 remain unchanged: decimal lifetimes and accent colors are host-local preferences. Unit coverage accepts every tenth from 0.1 through 30, rejects invalid lifetimes/colors, checks a 500 ms inactivity deadline with interaction renewal, and preserves legacy preferences and custom colors across hosting/restarts.

`npm run test:visual` passed on Chrome 152.0.7977.78 and Edge 153.0.4234.32. New light/dark fixtures verify fonts, individual borders, padding, corner radii, transparent/translucent backgrounds, hidden native controls with visible labels, readable ARIA option cards, exclusive virtual radio groups, compact text-bearing controls, live theme/color changes and transformed geometry. Existing value/focus/selection/event isolation, password masking, scroll clipping, removal and cleanup checks still pass. Page screenshots were inspected, including the light/dark comparison and a custom-color theme update. The UI script passed seven screenshot scenarios and checks pre-session preferences, all five presets, a custom color and rejection of invalid decimal input.

Native-session checks passed Chrome→Chrome, Edge→Edge and Chrome→Edge, including custom-color changes visible in received WebRTC video while simulated text is preserved, plus 0.5-second expiry in root documents and embedded questionnaires. The first combined smoke run timed out checking a Live-control click in the second authorized tab on Chrome→Edge; the isolated Chrome→Edge rerun passed without an application change. This records the retry rather than treating the initial batch as entirely successful. `npm run test:frames` passed on both browsers, and `npm run test:questionnaires` passed all 40 Live-control comparisons.

These fixtures validate computed-style adaptation, not pixel-perfect reproduction of arbitrary real sites. Background images, complex pseudo-elements and advanced editors retain the documented limits. Existing multi-computer, native-permission and publication checks below remain separate.
