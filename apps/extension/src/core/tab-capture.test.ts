...
describe('TabCapture authorization and scoped control', () => {
  ...
  it('changes interaction modes without changing presentation and rejects stale input or navigation', async () => {
    ...
    await h.capture.configure({ mode: 'live', revision: 1, preferences: { notices: false, text: { duration: 'persistent', seconds: 10 }, other: { duration: 'persistent', seconds: 3 }, accentColor: '#7871e8' } });
