...
const move = (x: number): ControlCommand => ({ type: 'pointer', ...target, event: 'move', x, y: 20, button: 'left', buttons: 1, modifiers: 0, clickCount: 0 });
function harness() {
...
it('flushes the final move before discrete input without dropping repeated text', () => {
  const h = harness(), text: ControlCommand = { type: 'text', ...target, text: 'a' };
...
it('discards pending movement on cancellation and presentation replacement', () => {
  const h = harness(), release: ControlCommand = { type: 'input.release', ...target };
