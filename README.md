...
- **Simulation preferences:** In the host menu, choose **Until cleared** (default) or **Temporary**, with 1–30 seconds of inactivity (initially 3). **Clear simulation** removes all previews. **Simulation notices** are off by default; enabled notices last 1.5 seconds and appear at most once every 3 seconds. These preferences survive restarts independently of connection settings. Preview text is kept only in memory and is cleared on navigation, tab/mode changes, pause, control withdrawal or session end. Escape releases keyboard capture while preserving previews.
