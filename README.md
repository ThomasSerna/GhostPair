...
Version 0.5.0 uses peer protocol 4 for visual simulation and interaction revisions. Update both participants together; saved addresses, identities and settings are preserved.
...
- **Visual simulation (default):** Each session starts in **Visual only**. Both participants see simulated clicks, button presses, checkbox/radio choices and text on a separate presentation layer. Simulated guest actions leave page values, focus and click/keyboard/input handlers untouched. Local host editing focuses the extension-owned editor. Links and buttons do not navigate or advance forms in this mode. Scrolling, GhostPair navigation/tab controls and optional clipboard synchronization remain real. The host can select **Live control** from **Interaction mode** during the session.
- **Simulation preferences:** Configure preferences in Settings before hosting or in the host menu during a session. Text and other simulations each have independent **Until cleared** (default) or **Temporary** settings, with 0.1–30 seconds of inactivity in steps of 0.1 (initially 10 for text and 3 for other previews). Existing duration settings migrate to both categories. Activity and expiration affect only their category across authorized frames; click pulses and notices remain brief. Choose purple (default), blue, green, orange, pink or a custom color; changing color preserves existing previews. **Clear simulation** removes all previews. **Simulation notices** are off by default; enabled notices last 1.5 seconds and appear at most once every 3 seconds. These preferences save automatically and survive restarts independently of connection settings. Preview text is kept only in memory and is cleared on navigation, tab/mode changes, pause, control withdrawal or session end. Escape releases keyboard capture while preserving previews.
- **Host editing of simulated text:** Click a simulated field locally to edit or select its text. Copy, cut and paste use normal local clipboard gestures without enabling clipboard synchronization. Drag selected text within or between simulated fields, including accessible frames; hold Ctrl to copy instead of move. A fresh destination receives a preview, while the original page values remain unchanged. Text is limited to 256 KiB per field; passwords remain masked and cannot be copied or dragged. Remote input waits up to five seconds for local IME composition, then reports an error if composition continues. Escape leaves the local editor; Tab moves among supported text fields.
- **Click animations:** Turn **Click animations** off in Settings or the host menu to hide all decorative click circles, including any currently animating. Button presses, choices, simulated text and notices keep working. The preference defaults to on, saves automatically and applies to authorized frames without clearing responses.
- **Page-aware appearance:** Text previews follow the page's fonts, spacing, individual borders, rounded corners and solid or translucent background colors. Choices reproduce readable page styles in isolated, passive replicas, including native controls, ARIA choices, hidden inputs, separate labels, selected option cards, sibling selectors, `:has()`, pseudoelements and inline SVG. Radio changes also redraw the previous answer as unselected. Page colors take priority over the simulation accent; button presses and focus use available page styles. Theme and layout changes update existing previews. Inaccessible styles or JavaScript-managed selection designs receive a visible fallback based on the control's shape and colors. External image assets, complex editors and closed widgets may differ.
...
`GET /health` reports process liveness; `GET /ready` also checks identity storage. Neither includes session content. The server does not log message bodies or passwords. Monitor process health, memory, disk capacity, and connection failures; do not enable payload logging in the proxy.
